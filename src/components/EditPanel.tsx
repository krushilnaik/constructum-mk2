...
export function EditPanel({
  ...
  return (
    <div className="fixed left-0 top-0 w-sidebar h-full bg-white shadow-xl border-r border-gray-200 z-10 transform transition-transform duration-300 ease-out animate-in slide-in-from-left">
          ...
          <div className="space-y-3 px-1 pr-4">
            {(task.depends_on || []).map((depId, idx) => (
              <div key={idx} className="flex gap-2 items-center">
                ...
                <button
                  onClick={() => {
                    const newDeps = (task.depends_on || []).filter((_, i) => i !== idx);
                    const newTypes = new Map(dependencyTypes);
                    newTypes.delete(depId);
                    updateDependencies(newDeps, newTypes);
                  }}
